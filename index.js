(async function() {
    ...
    function setCssCustomProperties() {
        if (config) {
            document.documentElement.style.setProperty('--gradient-color-1', config.gradient1);
            document.documentElement.style.setProperty('--gradient-color-2', config.gradient2);
        }
    }
    ...
    function updateInitialTexts() {
        if (t && config && elements.headerTitle) { // Ensure elements are also available
            ...
            elements.emailInputArea.querySelector('p').textContent = t["Please enter your email address to start a conversation with our support team."];
            elements.emailInput.placeholder = t["Enter your email address"];
            elements.emailSubmitBtn.textContent = t["Start Conversation"];
            elements.typingIndicatorBubble.querySelector('.typing-label').textContent = t["Support Team"];
            elements.msgInput.placeholder = t["Type your message..."];
            elements.inputStatusMessage.textContent = t["Please choose an option to continue."];
        ...
        setTimeout(() => {
            ...
            setTimeout(async () => {
                if (tab === 'help' && view === 'articleContent' && elementId) {
                    const articleToDisplay = articles.find(article => article._id === elementId);
                    if (articleToDisplay) {
                        showView('articleContent', 'right', { title: articleToDisplay.title, description: articleToDisplay.description });
                        // Assuming 'marked' is globally available, if not, load it or use a simpler parser
                        const response = await fetch(`${config.backendUrl}/api/websites/faqs/${config.chatbotCode}/${articleToDisplay._id}`)

                        const data = await response.json();
                    
                        const answer = data.answer;

                        const markdownToHtml = typeof marked !== 'undefined' ? marked.parse(answer) : answer;
                    ...
                    if (elementId === 'null' || elementId === null) { // Check if elementId is explicitly null or the string 'null'
    ...
    function initializeDeepLinkButtons() {
        ...
        deepLinkButtons.forEach(button => {
            ...
            if (!ariaLabel) {
                console.warn("Button with id 'cbh-deep-link' is missing an 'aria-label'. Skipping deep-link processing for this button.", button);
                return;
            }
            ...
            button.addEventListener('click', (event) => {
                ...
                if (ariaLabel.startsWith('http')) {
                    ...
                    if (parts.length < 2) {
                        console.error("Invalid aria-label format for URL deep link:", ariaLabel);
                        return;
                    }
                    const url = parts[0];
                    ...
                    if (target === 'new') {
                        ...
                        console.warn("Unknown target specified in aria-label for URL deep link:", target);
                    ...
                    const parts = ariaLabel.split(separator);
                    if (parts.length !== 3) {
                        console.error("Invalid aria-label format for deepLink function. Expected 'tab->*cbhdeeplink^&^cbhdeeplink*->view->*cbhdeeplink^&^cbhdeeplink*->elementId'", ariaLabel);
                        return;
                    }
                    const tab = parts[0];
                    const view = parts[1];
                    ...
                    if (typeof deepLink === 'function') {
                        ...
                        console.error("The 'deepLink' function is not defined. Cannot perform internal deep link.", ariaLabel);
                    }
                }
            });
        });
    ...
    function addInputFocusEffects() {
        if (!config || !elements.emailInput || !elements.msgInput || !elements.emailSubmitBtn) return;
    ...
    const createMessageBubble = (sender, text, timestamp, options = [], isReplySent = false, fileUrl) => {
        ...
        let markdownToHtml = typeof marked !== 'undefined' ? marked.parse(text) : text;

        // Modify all <a> tags to include target="_blank"
        markdownToHtml = markdownToHtml.replace(/<a\s+(?!.*target=["']_blank["'])([^>]*?)>/gi, '<a $1 target="_blank">');
                ...
                if (isReplySent) {
                    ...
                    optionButton.addEventListener('click', () => {
                        if (state.currentChatId) {
                            ...
                            const allButtonsInThisBubble = optionsContainer.querySelectorAll('.option-button');
                            ...
                            if (socket) { // Ensure socket is defined
                                socket.emit("message", {
    ...
    const showTypingIndicator = () => {
        if (state.isTyping || !elements.typingIndicatorBubble || !elements.messagesContainer) return;
        if (elements.messagesContainer.lastChild !== elements.typingIndicatorBubble) {
    ...
    const updateHeaderForView = (view, title, description) => {
        if (!elements.headerTitle || !elements.headerSubtitle || !elements.backBtnArtilces || !elements.backBtnChats || !elements.footer) return;
        ...
        switch (view) {
            ...
            case 'conversations':
                ...
                elements.headerSubtitle.textContent = t['Select a chat or start new one'];
                elements.backBtnArtilces.style.display = 'none'
                elements.backBtnChats.style.display = 'none';
                displayFooter("flex");
            ...
            case 'articles':
                ...
                elements.headerSubtitle.textContent = t['Find answers to common questions and get help with using our platform.'];
                elements.backBtnArtilces.style.display = 'none'
                elements.backBtnChats.style.display = 'none';
                displayFooter('flex');
                elements.footer.classList.remove('hidden');
    ...
    const showView = (viewName, direction = 'right', headerProps) => {
        ...
        setTimeout(() => {
            ...
            if (targetView) {
                targetView.style.display = (viewName === 'email' || viewName === 'articles' ) ? 'flex' : 'block';
    ...
    const updateInputAreaVisibility = (showInput) => {
        if (!elements.fileInputContainer || !elements.inputStatusMessage || !elements.inputArea || !elements.sendBtn) return;
    ...
    function renderArticles(filteredArticles) {
        ...
        if (filteredArticles.length === 0) {
            elements.articleScrollableContainer.innerHTML = '<p style="text-align: center; color: var(--text-color-secondary); padding: 20px;">No articles found.</p>';
            return;
        }
        filteredArticles.forEach(article => {
            ...
            articleCard.addEventListener('click', async () => {
                showView('articleContent', "right", { title: article.title, description: article.description });
                
                const response = await fetch(`${config.backendUrl}/api/websites/faqs/${config.chatbotCode}/${article._id}`)
    ...
    const loadMessages = async (chatId) => {
        ...
        try {
            ...
            if (state.currentChatId === chatId && t) { // Only show error if still on the same chat
    ...
    const loadUserChats = async (email) => {
        ...
        try {
            const response = await fetch(`${config.backendUrl}/api/chats/${config.chatbotCode}/${email}`);
            if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
            ...
            if (chats.length === 0) {
                ...
                sortedChats.forEach(chat => {
                    ...
                    const createdAtDate = new Date(chat.createdAt).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', month: 'short', day: 'numeric', year: 'numeric' });
                    const updatedAtDate = new Date(chat.updatedAt).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', month: 'short', day: 'numeric', year: 'numeric' });

                    chatItem.innerHTML = `
                        <div class="chat-header-info">
                            <strong>${chat.name}</strong>
                            <span class="status-tag ${chat.status}">${t[chat.status]}</span>
                        </div>
                        <div class="chat-meta">📅 ${t["Created:"]} ${createdAtDate}</div>
                        <div class="chat-meta">🕒 ${t["Last Update:"]} ${updatedAtDate}</div>
                        <div class="chat-view-prompt">💬 ${t["Click to view conversation"]}</div>
                    `;

                    chatItem.addEventListener('click', () => handleShowChatMessages(chat._id));
    ...
    function setupSocketListeners() {
        ...
        if (!config || typeof io === 'undefined') {
            console.error("Attempted to set up socket listeners before config was loaded or Socket.IO client library is missing.");
            return;
        }
        socket = io(config.socketIoUrl, {
            path: '/socket.io',
            query: { chatbotCode: config.chatbotCode, currentWebsiteURL: window.location.href },
    ...
    async function initializeChatbot() {
        ...
        if (!elements.chatButton || !elements.chatWindow || !elements.chatbotWidget) {
            console.error('CRITICAL: Essential chatbot DOM elements not found. Initialization stopped.');
        ...
        const response = await fetch(`${config.backendUrl}/api/websites/faqs/${config.chatbotCode}`)
        ...
        if (elements.closeBtns) {
            elements.closeBtns.forEach(btn => btn.addEventListener('click', handleToggleWidget));
        }
        ...
        if (elements.backBtnArtilces) {
            elements.backBtnArtilces.addEventListener('click', () => showView('articles', 'left'));
        }

(async function () {
    ...
    try {
      ...
      socket.emit("create_new_chat", {
        ...
        country: {
          ...
          countryCode: data.success ? data.country_code : "",
          flag: data.success ? data.flag.img : "",
        },
      });
    } catch (error) {
    ...
    socket.on("chat_update", (data) => {
      ...
      if (data.chatId === state.currentChatId) {
        ...
        if (data.status === "closed") {
          updateInputAreaVisibility(false);
          if (t && !data.message) {
            // Only add 'closed' message if no other message is provided
            renderMessage(
              "bot",
  ...
  function attemptInitialization() {
    ...
    if (domReady && config !== null) {
      console.log(
        "--- Both DOM ready and config loaded. Proceeding with initialization. ---"
      );
      initializeChatbot();
    }
  }
